#! /bin/bash

curl -L https://omnitruck.chef.io/install.sh | sudo bash
cd /etc/
sudo mkdir chef
cd /etc/chef
sudo vim client.rb
echo 'chef_server_url  "https://chef.ascendingdc.com/organizations/fin_org"
validation_client_name "fin_org-validator"
log_location   STDOUT
node_name "client-3"
validation_key          "/etc/chef/validation.pem"
# Using default node name (fqdn)' >client.rb
sudo vim validation.pem
echo '
1PIr2GEMnhcvlgj2uHTEjniK4Qg63w1JMRNkDSGvr6D9q4RAW8J9' >validation.pem
#sudo chef-client --once